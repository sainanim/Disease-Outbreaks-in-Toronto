...
cleaned_data <-
  raw_data %>% mutate(across(everything(), ~ifelse(is.na(.), NA, .))) %>%
  
  # Convert date columns to Date format
  mutate(Date_Outbreak_Began = as.Date(Date_Outbreak_Began, format = "%Y-%m-%d"),
         Date_Declared_Over = as.Date(Date_Declared_Over, format = "%Y-%m-%d")) %>%
  ...
  mutate(across(c(Outbreak_Setting, Type_of_Outbreak, Causative_Agent_1), as.factor))
